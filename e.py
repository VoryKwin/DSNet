...
if __name__ == '__main__':
    args = parse_args()
    test_data = load_test_data(args)  # 一次加载测试数据

    while True:  # 无限循环，直到用户输入 'q' 才退出
        network_input = input("Enter the network address or 'q' to quit: ")
        if network_input.lower() == 'q':
            break  # 如果用户输入 'q'，退出循环

        args.network = [network_input]
