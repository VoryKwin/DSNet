...
class GraspRectangles:
    ...
    def load_from_cornell_file(cls, fname):
        ...
        with open(fname) as f:
            while True:
                # 一次加载四行，表示一个抓取框的四个角点坐标
                p0 = f.readline()
                if not p0:
                    break  # EOF
                p1, p2, p3 = f.readline(), f.readline(), f.readline()
                try:
                    # 将四个角点的坐标转换为数组，构成一个抓取矩形
                    gr = np.array([_gr_text_to_no(p0), _gr_text_to_no(p1), _gr_text_to_no(p2), _gr_text_to_no(p3)])

                    # 创建一个 GraspRectangle 的实例，并添加到抓取矩形列表中
        ...
        # 使用抓取矩形列表创建一个 GraspRectangles 的实例并返回
        return cls(grs)
