...
def parse_args():
    ...
    parser.add_argument('--dataset-path', type=str, default="F:\GraspLab\datasets\jacquard",
...
if __name__ == '__main__':
    ...
    test_dataset = Dataset(args.dataset_path, start=args.split, end=0.98915, ds_rotate=args.ds_rotate,
                           ds_shuffle=args.ds_shuffle, random_rotate=False, random_zoom=False,
    ...
    logging.info('Done')
    network_input = input("Enter the path to the initial network (or press Enter to exit): ")
    networks = []
    networks.append(network_input)
    for network in networks:
        ...
        # 等待用户输入下一个网络权重的地址
        next_network = input("Enter the path to the next network (or press Enter to exit): ")
